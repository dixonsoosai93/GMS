...
@WebServlet("/EditDtDailyDairy")
public class EditDtDailyDairy extends HttpServlet {
	private static final long serialVersionUID = 1L;
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(EditDtDailyDairy.class.getName());
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        DtDailyDairy.setDAIRYID(DIARYID);
        ...
        System.out.println("Insert Data:"+DtDailyDairy.toString());
        try {
            boolean success = DailyDairyDAO.updateDailyDairy(DtDailyDairy);
            String url;
            if(success) {
                ...
                session.setAttribute("authError", "Error in update Daily Dairy Master");
                url = "DailyDairyUpdationList.jsp";
            }
            response.sendRedirect(url);
        } catch (Exception ex) {
        	ex.printStackTrace();
        	Logger.getLogger(EditDtDailyDairy.class.getName()).log(Level.SEVERE, null, ex);
        }
