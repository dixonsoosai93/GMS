...
@WebServlet("/AlternateInvReAssign")
public class AlternateInvReAssign extends HttpServlet {
	private static final long serialVersionUID = 1L;
	static Logger logger = Logger.getLogger(AlternateInvReAssign.class.getName());
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        try {
	    	...
	    	GrievanceDesc grievanceDesc=AlternateInvReAssignDAO.getGrievanceDesc(GRVDETID);
	    	List<GridCase> gridCase=AlternateInvReAssignDAO.getGridCase(GRVDETID);
	    	...
	    	if(grievanceDesc != null) {
	    		...
	    		jsonObject.put("ZONE_CODE", grievanceDesc.getZONE_CODE());
	    		jsonObject.put("REGION_CODE", grievanceDesc.getREGION_CODE());
	    	}
	    	...
	    	if(gridCase != null && gridCase.size() >0) {
	    		...
	    		for(GridCase item : gridCase) {
	    			...
	    			grid_case+="<td>"+(item.getRE_ASSIGN().equals("Y")?"Yes":"No")+"</td>";
	    			...
	    			grid_case+="<td>"+((item.getCURRENT_ROW_COMPLETED().equals("N")) && (item.getFlagButton().equals("A"))==true? "<button type=\"button\" data-key1='"+item.getHIERORDER()+"' data-key2='"+item.getCASEID()+"' class=\"btn btn-primary changeInv\">Change Redressal Authority</button>":"")+"</td>";
	    			i++;
	    			}
	    		}
	    		...
	    		Logger.getLogger(AlternateInvReAssign.class.getName()).log(Level.SEVERE, null, ex);
