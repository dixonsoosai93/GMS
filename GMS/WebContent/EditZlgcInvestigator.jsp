        ...
        <div class="vz_main_content">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card_title text-right">Edit ZLGC Redressal Authority</h4>
                             <%
	                            int Id = Integer.parseInt(request.getParameter("ZLGC"));
                                ZlgcInvestigatorMasterModel zlgcInvestigator =  ZlgcInvestigatorDAO.getzlgcInvestigator(Id);
	                         	 %> 
                            <form id="ZlgcInvestigator" action="EditZlgcInvestigatorMaster" method="post">
                               <div class="form-group">
                               	 <label class="col-form-label" for="EZLGC">ZLGC ID</label>
                                 <input type="text" name="EZLGC" class="form-control" value="<%=request.getParameter("ZLGC")%>" readonly required>
                                 </div>
	                             <div class="form-group">
								   <label class="col-form-label" for="EZONE_CODE">Zone Name</label>
									<select class="js-select2 form-control" name="EZONE_CODE" id="EZONE_CODE"  aria-hidden="true" value="<%=zlgcInvestigator.getZONE_CODE()%>">
										<option selected="selected" disabled>Choose Zone Name</option>
										<%
										int j=1;
										List<ZoneModel> Zone=HrcMasterDAO.getZone();
										   if(Zone != null){
				                             	for(ZoneModel item : Zone){
					        				%>
					      					<option value="<%=item.getZONE_CODE()%>" <% if(item.getZONE_CODE().equals(zlgcInvestigator.getZONE_CODE())) { %>selected<% } %>><%=item.getZONE_NAME()%></option>
											<%
								             j++;
								         	} 
				                            }
	                            ...
	                            <div class="form-group">
	                                <label class="col-form-label" for="EZONE_ID">Zone ID</label>
	                                <input type="text" name="EZONE_ID" class="form-control" id="EZONE_ID"  value="<%=zlgcInvestigator.getZONE_CODE()%>" disabled>
	                            </div>			                      
	                            <div class="form-group">
	                                <label class="col-form-label" for="EINVESTIGATOR_EMPLOYEE_ID">Redressal Authority Employee Name</label>
	                                <div class="d-flex">
	                                <select class="js-select2 form-control" name="EINVESTIGATOR_EMPLOYEE_ID" id="EINVESTIGATOR_EMPLOYEE_ID" aria-hidden="true" value="<%=zlgcInvestigator.getNAME()%>">
										...
										List<ManpowerModel> Manpower2=HrcMasterDAO.getManpower();
										   if(Manpower2 != null){
					        				...
					        				%>
					      					<option value="<%=item.getEMPLOYEE_ID()%>" data-key1 ='<%=item.getDESIGNATION() %>' data-key2 ='<%=item.getDEPARTMENT()%>' <% if(Integer.parseInt(item.getEMPLOYEE_ID())==zlgcInvestigator.getINVESTIGATOR_EMPLOYEE_ID()) { %>selected<% } %>><%=item.getNAME()%></option>
											<%
								             j++;
								         	} 
				                            }
	                            ...
	                            <div class="form-group">
	                                ...
	                                <input type="text" name="EEMPLOYEE_ID" class="form-control" id="EEMPLOYEE_ID" value="<%=zlgcInvestigator.getINVESTIGATOR_EMPLOYEE_ID()%>" disabled>
	                            </div>
	                            <div class="form-group">
	                                <label class="col-form-label" for="EDESIGNATION">Redressal Authority Employee Designation</label>
	                                <input type="text" name="EDESIGNATION" class="form-control" id="EDESIGNATION" disabled value="<%=zlgcInvestigator.getDESIGNATION()%>">
	                            </div>
	                            <div class="form-group">
	                                <label class="col-form-label" for="EDEPARTMENT">Redressal Authority Employee Department</label>
	                                <input type="text" name="EDEPARTMENT" class="form-control" id="EDEPARTMENT" disabled value="<%=zlgcInvestigator.getDEPARTMENT()%>">
	                            </div>
	                            <div class="form-group">
	                                ...
	                                <select class="custom-select" id="EIS_HR" name="EIS_HR">
                                        ...
                                        <option value="Y" <% if(zlgcInvestigator.getIS_HR().equalsIgnoreCase("Y")){ %> Selected <% } %>>Yes</option>
                                        <option value="N" <% if(!zlgcInvestigator.getIS_HR().equalsIgnoreCase("Y")){ %> Selected <% } %>>No</option>
	                                </select>
	                                <span style="color: indianred; float: right;"> *</span>
	                            </div>
                                <div class="form-group">
                                    ...
                                    <select class="custom-select" name="EIS_ACTIVE">
                                        <option value="Y" <% if(zlgcInvestigator.getIS_ACTIVE().equalsIgnoreCase("Y")){ %> Selected <% } %>>Yes</option>
                                        <option value="N" <% if(!zlgcInvestigator.getIS_ACTIVE().equalsIgnoreCase("Y")){ %> Selected <% } %>>No</option>
                                    </select>
                                </div>   
...
<script>
	$("#EZONE_CODE").change(function(){
		 var EZONE_ID =$("#EZONE_CODE").val();
		 $('#EZONE_ID').val(EZONE_ID);
	});
...
<script>
    ...
    $('.cancel').click(function(){
   		location.href ="ZlgcInvestigatorList.jsp"; 
   	}); 
</script>
</body>
</html>